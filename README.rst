...
浏览器解析URL一般分为以下几步，不同浏览器会略有不同。

1. 判断URL是绝对(absolute)URL，还是相对（relative）URL。在大部分浏览器里，手动在地址栏里输入的地址，全部视为绝对URL。
2. 确定URL为绝对URL时，开始解析。一个标准的绝对URL结构是
  ``
  scheme://login:password@address:port/path/to/resource?query_string#fragment
  ``
3. 提取协议名称：查找第一个"："，该字符左边的部分就是协议名称，如果协议名称里出现了不该有的字符，则该URL就是相对URL，这部分内容也不会当成协议处理。
4. 去除层级标记符：如果有"//"就跳过，在实际环境里，不用斜杠用一个用两个用三个其实都可以正常解析。
5. 获取授权部分信息（登录信息+地址+端口）：依次查找“/”、“？”、“#”，哪个先出现以哪个为准截取，除了IE和Safari，分号；也是可接受的分隔符。
	- 授权部分信息提取出来后，先查找@符号，如果找到了，那它前面的部分就是登录信息，冒号前面是用户名，后面是密码
	- 剩下部分就是地址了，查找冒号：，如果有则前面的部分是地址，后面部分是端口，如果发现方括号[]，说明地址是IPv6
6. 确定``/path/to/resourc``路径
7. 提取``query_string ``查询字符串
8. 提取``fragment ``片段ID


输入的是 URL 还是搜索的关键字？
-----------------------------
地址栏直接识别搜索关键词是近些年的新浏览器的功能，具体实现在上述3-4步之间发生，不同浏览器也各有不同，以下以Chrome举例：

 1. Chrome会先判断第一个``:``是否存在，如果存在跳至2，不存在跳至4。
 2. 判断协议是否有效，[IANA有效协议列表](http://www.iana.org/assignments/uri-schemes/uri-schemes.xhtml) ，协议有效跳至3，协议无效跳至4。
 3. 如果协议有效，判断是否为常用协议，常见协议有http、https、ftp，还有几个伪协议data、javascript、about、chrome，如果是常见协议按照协议处理，如果不是跳至4.
 4. 浏览器首先检查最后一个``.``之后字符串是否为通用顶级域，如com、edu、us、cn、info等等，如果有顶级域，浏览器会自动补充完整``http``协议和``//``层级标记符，把该url当成普通http请求url继续处理，否则当成搜索关键词处理。
 5. 当确认用户输入的是搜索关键词时，先读取浏览器配置，将用户输入内容组装到默认搜索引擎搜索URL的``query_string``里，再去进一步处理。

 当然，Chrome在你输入的过程中，除了本身默认的解析，地址栏下方还会给你其他备选项，你可以按``↑``和``↓``来选择你希望的操作。
