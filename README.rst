...
键盘在它的中断请求线(IRQ)上发送信号，信号会被中断控制器映射到一个 ``中断向量`` ，实际上就是一个整型数 。CPU使用``中断描述符表(IDT)`` 把中断向量映射到对应函数，这些函数被称为``中断处理器``，它们由操作系统内核提供。当一个中断到达时，CPU按照IDT和中断向量索引到对应的中端处理器，然后操作系统内核出场了。
